...
def all_techs_model(number_of_time_steps=365 * 24,
    ...
    demand1 = pd.read_csv(os.path.join(dir_path, r'C:\Users\dhun_go\Pycharm_Projects\enaq-simulation\data\demands\demand_el_heat.csv'),
                            comment='#', index_col=0,
                            sep=',',
                            parse_dates=True)
    ...
    time_series = {
        ...
        'demand': {
            'electricity': data['electricity_sum'],  # MW (time series)
            'heating': data['KFW_40'],  # MW (time series)
