...
class TestDataHandler:
    def test_list(self, data_handler):
        data_list = [1, 2, 3, 4, 5]
        data = data_handler.get_timeseries(data_list, kind="point")
    ...
    def test_series(self, data_handler):
        ...
        point_data = data_handler.get_timeseries(data_series, kind="point")

        with pytest.raises(ValueError):
            # series to long for interval data
            data_handler.get_timeseries(data_series, kind="interval")
        ...
        with pytest.raises(ValueError):
            ...
            data_series = pd.Series(data=data_list)
            data_handler.get_timeseries(data_series, kind="point")
            data_handler.get_timeseries(data_series, kind="interval")
        ...
        interval_data = data_handler.get_timeseries(data_series, kind="interval")
    ...
    def test_series_with_timeindex(self, date_range, data_handler):
        ...
        with pytest.raises(KeyError):
            # '2021-07-10 00:00:00' not coveres
            data_handler.get_timeseries(data_series, kind="point")
