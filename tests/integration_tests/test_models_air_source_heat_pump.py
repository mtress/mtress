...
def test_air_heat_pump_r0_b0():
    ...
    ahp_generation = meta_model.aggregate_flows(meta_model.ahp_th_flows).sum()
    ...
    assert math.isclose(ahp_generation, heat_demand.sum(),
...
def test_air_heat_pump_r30_b30():
    heat_demand = np.full(3, 0.1)
    design_cop = 3
    electricity_demand = heat_demand/design_cop
